...
if(!isset($_SESSION['apUserId']))
{
    ...
    $colorsQuery = "SELECT * FROM `colors` WHERE `color_id` = :id";
    $colorsPrepare = $connect->prepare($colorsQuery);
    $colorsPrepare->bindParam(":id", $id, PDO::PARAM_INT);
    $colorsPrepare->execute();
    $colorsData = $colorsPrepare->fetch(PDO::FETCH_ASSOC);
  ...
  else
  {
    ...
    $colorQuery= "UPDATE `colors` SET `color_name`= :color_name WHERE `color_id` = :id";

    $colorprepare = $connect->prepare($colorQuery);
    $colorprepare->bindParam(':color_name',$ColorName,PDO::PARAM_STR);
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <div class="row">
                <div class="col-md-12">
                  ...
                  <div class="card mb-4">
                    <h5 class="card-header">Edit Colour</h5>
                    <!-- Account -->
                      ...
                      <form id="formAccountSettings" method="POST" enctype="multipart/form-data">
                        <div class="row">
                          <div class="mb-3 col-md-12">
                            <label for="firstName" class="form-label">Colour Name</label>
                            <input
                              class="form-control"
                              type="text"
                              id="firstName"
                              ...
                              required
                              value="<?= $colorsData['color_name'] ?>"
                            />
                          </div>
                          
                        </div>
                        <div class="mt-2">
                          <button type="submit" class="btn btn-primary me-2" name="btn">Edit Colour </button>
