...
$viewQuery = "SELECT * FROM `cities` JOIN `countries` ON cities.country_id = countries.country_id WHERE `city_id` = :id";
...
$viewQuery = "SELECT * FROM `countries`";
$viewPrepare = $connect->prepare($viewQuery);
$viewPrepare->execute();
...
if(isset($_POST['btn']))
{
  ...
  else
  {
    ...
    $countryId = $countryName ?? $singleCityData['country_id'];
    
    if (!preg_match("/^[A-Za-z ]+$/", $cityName)) {
      echo "<script>alert('Error: City Name should only contain alphabets!');</script>";
    } else {
      $insertQuery= "UPDATE `cities` SET `city_name`= :cityName , `country_id`= :countryName WHERE city_id = :id";

      $insertPrepare = $connect->prepare($insertQuery);
      $insertPrepare->bindParam(':cityName',$cityName,PDO::PARAM_STR);
      $insertPrepare->bindParam(':countryName',$countryId ,PDO::PARAM_STR);
                    ...
                    <div class="card-body">
                      <form id="formAccountSettings" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3 col-md-12">
                            ...
                            <select id="country" class="select2 form-select" name="countryName">
                              <option selected deslected style="font-weight: bolder;" value="<?= $singleCityData['country_id'] ?>"><?= $singleCityData['country_name'] ?></option>
                              <?php foreach ($countryData as $c) {  
                                  if($c['country_name'] != $singleCityData['country_name']){
                                ?>
                          ...
                          <div class="mb-3 col-md-12">
                            <label for="firstName" class="form-label">City Name</label>
                            <input
                              class="form-control"
                              type="text"
                              id="firstName"
                              ...
                              required
                              value="<?= $singleCityData['city_name'] ?>"
                            />
                          </div>
                          
                        </div>
                        <div class="mt-2">
                          <button type="submit" class="btn btn-primary me-2" name="btn">Edit City</button>
