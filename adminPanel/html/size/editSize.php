...
if(!isset($_SESSION['apUserId']))
{
    ...
    $sizeQuery = "SELECT * FROM `sizes` WHERE `size_id` = :id";
    $sizePrepare = $connect->prepare($sizeQuery);
    $sizePrepare->bindParam(":id", $id, PDO::PARAM_INT);
    $sizePrepare->execute();
    $SizeData = $sizePrepare->fetch(PDO::FETCH_ASSOC);
...
if(isset($_POST['btn'])){
    ...
    else
    {
      ...
      $SizeQuery= "UPDATE `sizes` SET size_name = :size_name WHERE size_id = :id";
  
      $SizeQuery = $connect->prepare($SizeQuery);
      $SizeQuery->bindParam(':size_name',$SizeName,PDO::PARAM_STR);
      $SizeQuery->bindParam(':id',$id,PDO::PARAM_STR);
  
      if($SizeQuery->execute()){
                      ...
                      <form id="formAccountSettings" method="POST" enctype="multipart/form-data">
                        <div class="row">
                          <div class="mb-3 col-md-12">
                            <label for="firstName" class="form-label">Size Name</label>
                            <input
                              class="form-control"
                              type="text"
                              id="firstName"
                        ...
                        <div class="mt-2">
                          <button type="submit" class="btn btn-primary me-2" name="btn">Edit Size</button>
